...
class ClipB16FeatureExtractor(BaseFeatureExtractor):
    ...
    def forward(self, x, y=None, fusion_type=False):
        ...
        if y is not None:
            ...
            for y_i in y:
                ...
                txt_features.append(txt_feature.squeeze())
            txt_features = torch.stack(txt_features, dim=0)
            # txt_features = txt_features * image_features.shape[0]
            
            if fusion_type:
                txt_ft = 0.4*txt_features[:, 0, :]+0.3*txt_features[:, 1, :]
                txt_ft = txt_ft / txt_ft.norm(dim=1, keepdim=True)
                # print(f'img:{image_features.shape}  y:{txt_ft.shape}')
                fusion_features = 0.3 * image_features + txt_ft
                # fusion_features = 0.3* image_features + 0.4 * txt_features[0] + 0.3 * txt_features[1]
                fusion_features = fusion_features / fusion_features.norm(dim=1, keepdim=True)
                return fusion_features.to(x.device)
            return (image_features.to(x.device), txt_features.squeeze().to(x.device))
            # return (image_features.to(x.device), txt_features.squeeze().to(x.device))
