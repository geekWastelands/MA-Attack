...
class EnsembleFtExtractor(BaseFeatureExtractor):
    def __init__(self, extractors: List[BaseFeatureExtractor]):
        super(EnsembleFtExtractor, self).__init__()
    ...
    def forward(self, x: Tensor, y: Any=None) -> Tensor:
        ...
        for model in self.extractors:
            if y==None:
                ...
                features.append(model(x,y,True))
...
class EnsembleFeatureLoss(nn.Module):
    ...
    def __call__(self, feature_dict: Dict[int, Tensor], weight: Any=[0.3, 0.5, 0.2], txt: bool = False) -> Tensor:
        loss = 0
        ...
        for index, model in enumerate(self.extractors):
            if txt==False:
                ...
                # print(f'gt:{self.ground_truth[index]}')
